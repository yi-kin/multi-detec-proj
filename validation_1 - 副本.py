...
def main(parser_data):
    ...
    with torch.no_grad():
        for image, targets in tqdm(val_dataset_loader, desc="validation..."):
            ...
            outputs_boxes = [t["boxes"] for t in outputs]
            outputs_labels = [t["scores"] for t in outputs]
            gt_boxes_after = []
            gt_labels_after = []
            outputs_after = []
            for outputs_boxes_in_image, outputs_labels_in_image,gt_boxes_in_image, gt_labels_in_image in zip(outputs_boxes,outputs_labels,gt_boxes, gt_labels):
                if (len(gt_labels_in_image) >= len(outputs_boxes_in_image)):
                    match_quality_matrix = box_ops.box_iou(outputs_boxes_in_image, gt_boxes_in_image)
                    ...
                    outputs_after.append(outputs_boxes_in_image)
                    output_list.extend(outputs_labels_in_image)
                else:
                    match_quality_matrix = box_ops.box_iou(gt_boxes_in_image, outputs_boxes_in_image)
                    _, indices = match_quality_matrix.max(dim=1)
                    outputs_boxes_in_image = outputs_boxes_in_image[indices]
                    outputs_labels_in_image = outputs_labels_in_image[indices]
