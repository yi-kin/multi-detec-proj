...
注意，这个xception是修改过的，增加了一层全连接层，并且在openforeniscs上进行了迁移学习
...
class new_xception(nn.Module):
    def __init__(self,xception):
        ...
        params = {}
        for name, param in self.xception.net.named_parameters():
...
#Xceptionmodel
    model1 = xception()
    trans_learned_xcep = new_xception(model1)

    cnn_sd = torch.load('0epoch-0.8856147043893843.pth', map_location="cpu")
    trans_learned_xcep.load_state_dict(cnn_sd)

    trans_learned_xcep = trans_learned_xcep.to(device)

    trans_learned_xcep.eval()
    ...
    with torch.no_grad():
        for image, targets in tqdm(FFIW_Test_dataset_loader, desc="validation..."):
            ...
            for i in range(len(outputs_after)):
                ...
                if len(face_imgs)>0:
                    ...
                    pred = trans_learned_xcep(face_imgs)
        ...
        print(f'FFIW | trans-learned-on-openfor->xception-AUC: {auc:.4f}')
    #############################################  val
        target_list = []
        output_list = []
        ...
        target_list = [*map(lambda x: x - 1, target_list)]
        auc = roc_auc_score(target_list, output_list)
        print(f'openfor | trans-learned-on-openfor->xception-Val- AUC: {auc:.4f}')
        ...
        target_list = [*map(lambda x: x - 1, target_list)]
        auc = roc_auc_score(target_list, output_list)
        print(f'openfor | trans-learned-on-openfor->xception-test- AUC: {auc:.4f}')
        #############################################  cha
        target_list = []
        output_list = []
        ...
        target_list = [*map(lambda x: x - 1, target_list)]
        auc = roc_auc_score(target_list, output_list)
        print(f'openfor | trans-learned-on-openfor->xception-Challenge- AUC: {auc:.4f}')
