...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    #model = (backbone(resnet50+fpn)+RPN+roiheads(detection part)+maskpart)
...
def main(args):
    ...
    det_results_file = f"det_results/det_results{now}.txt"
    seg_results_file = f"seg_results/seg_results{now}.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-path', default='data/openfrenic', help='dataset')
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=2, type=int, help='num_classes')
    # 文件保存地址
    parser.add_argument('--output-dir', default='./save_weights', help='path where to save')
    # 若需要接着上次训练，则指定上次训练保存权重文件地址
    parser.add_argument('--resume', default='save_weights/model_4.pth', type=str, help='resume from checkpoint')
    # 指定接着从哪个epoch数开始训练
