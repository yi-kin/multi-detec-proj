...
class AnchorsGenerator(nn.Module):
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            # shape: [grid_width*grid_height, 4] -> 特征图上每个cell中anchor对应的偏移量
            shifts = torch.stack([shift_x, shift_y, shift_x, shift_y], dim=1)

            # For every (base anchor, output anchor) pair,
            # offset each zero-centered base anchor by the center of the output anchor.
            # 将anchors模板与原图上的坐标偏移量相加得到原图上所有anchors的坐标信息(shape不同时会使用广播机制)
            # 之前生成的base_anchor是以0,0为原点的一系列（15）个anchor
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 获取每个预测特征层的尺寸(height, width)  -> 每个特征层的尺寸大小，返回一个尺寸列表
        grid_sizes = list([feature_map.shape[-2:] for feature_map in feature_maps])

        # 获取输入图像的height和width -> 这里images_size是每个batch之前为了适应比例进行缩放之前，每张照片的原始大小
        ...
        # 根据提供的sizes（64，,128,256...）和aspect_ratios（0.5,1.0，）生成anchors模板
        self.set_cell_anchors(dtype, device)

        # 计算/读取所有anchors的坐标信息（这里的anchors信息是映射到原图上的所有anchors信息，不是anchors模板）
        # 得到的是一个list列表，对应每张预测特征图映射回原图的anchors坐标信息
        # anchors_over_all_feature_maps包含所有特征图映射回来的anchors
...
class RPNHead(nn.Module):
    ...
    def forward(self, x):
        """
        这里用的是resnet50-fpn，所以需要在每一个特征层上预测类别和回归参数
        """
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def forward(self,
        ...
        # 筛除小boxes框（裁剪后），nms处理，根据预测概率获取前post_nms_top_n个目标
