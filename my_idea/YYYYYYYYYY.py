...
class AttentionLayer(nn.Module):
    ...
    # def forward(self, features, W_1, b_1, flag):  # feature:(4,2048)
        ...
        if flag == 1:
            ...
            # alpha01 = features.size(0) * losses.expand_as(features) #alpha0:(4,2048)
            context = torch.mul(features, m)
        else:
            context = features
            losses = torch.zeros(features.size(0), 1)
...
class MIL_xcep(nn.Module):
    ...
    def forward(self, x, targets = None,flag=1):
        _,mx = self.xception(x)

        out, out_c, alpha = self.att_layer(mx, targets,self.linear1.weight, self.linear1.bias,self.linear2.weight,self.linear2.bias, flag)
...
########################################
    for epoch in range(n_epoch):
           ...
           ########################这是将多余检测框去掉或是去掉多余的标注，方便后面计算AUC#######################################3
            ...
            for i in range(len(outputs_after)):
                ...
                if (len(face_imgs) > 1 and len(face_imgs)<10):
                    ...
                    optimizer.zero_grad()
                    bag_pre,instance_pre,alpha = model_cls(face_imgs,bag_label)
        ...
        print(f'FFIW | train-MIL-AUC: {auc:.4f}')
        ...
        with torch.no_grad():
            for image, targets in tqdm(test_dataset_loader, desc="testing..."):
                # 将图片传入指定设备device
            ...
            target_list = [*map(lambda x: x - 1, target_list)]
            auc = roc_auc_score(target_list, output_list)
            print(f'FFIW| Test-MIL-AUC: {auc:.4f}')
            print("count=", count)
            print("no-count", no_count)
