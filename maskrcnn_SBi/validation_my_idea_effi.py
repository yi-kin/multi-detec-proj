...
class ClassifierD(nn.Module):
    def __init__(self, efficient,input_dim=3, output_dim=2):
        super(ClassifierD, self).__init__()
        #########################
        self.xcep = efficient
        #############################
        self.linear1 = nn.Linear(3584, 128)
...
class ClsD(nn.Module):
    def __init__(self,efficient):
        ...
        self.input_dim = 3
        self.output_dim = 2
        self.net = ClassifierD(efficient,self.input_dim, self.output_dim)

        # Define loss function and optimizer
        self.criterion = nn.CrossEntropyLoss()
        params = {}
        for name,param in self.net.xcep.named_parameters():
            if name not in ["_fc.weight", "_fc.bias", '_bn1.weight', '_bn1.bias', '_conv_head.weight']:
                params[name] = param
...
#Xception--model############################
    ...
    # _,outputs = model1(x)  #outputs:(2,1792)
...
########################################
    for epoch in range(n_epoch):
        ...
        torch.save(model_cls.state_dict(),'./outputs/{}effi_new_epoch.pth'.format(epoch))
        torch.save(model1.state_dict(),'./outputs/{}effi_new_Xception_epoch.pth'.format(epoch))
